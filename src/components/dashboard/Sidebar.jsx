...
function MenuSection({ title, items, onNavClick }) {
...
function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar Panel */}
      <aside
        className={`flex flex-col overflow-y-auto overflow-x-hidden fixed z-50 bg-background sidebar-scrollbar items-center shadow-md py-4 transition-all duration-300 h-screen 
      ...
      >
        ...
        <div className="flex flex-col items-center px-4 mb-6">
          <div className="w-14 h-14 rounded-full bg-gray-200 flex items-center justify-center mb-2 overflow-hidden">
            {/* Avatar or initials */}
            {store.avatar ? (
              <img src={store.avatar} alt="avatar" className="w-full h-full object-cover" />
            ) : (
              <span className="text-2xl font-bold text-gray-600">
                {(store.firstName?.[0] || 'U').toUpperCase()}
              </span>
            )}
          </div>
          {!isCollapsed && (
            <>
              <div className="font-semibold text-text-primary text-center">
                {`${store.firstName || ''} ${store.lastName || ''}`.trim() || 'User'}
              </div>
              <div className="text-xs text-text-secondary mb-1">Account No: {store.accountNumber || 'N/A'}</div>
              {store.kyc && (
              ...
              <div className="flex gap-2 w-full">
                <Link to="/dashboard/profile" onClick={handleNavClick} className="flex-1 py-1 px-5 text-xs border rounded text-text-primary hover:bg-gray-100 text-center">Profile</Link>
        ...
        <nav className="flex-1 ">
          {menuSections.map((section) => (
            <MenuSection key={section.title} {...section} onNavClick={handleNavClick} />
          ))}
        </nav>
      </aside>
    </>
